...
function App() {
  ...
  useEffect(() => {
    juceBridge.whenReady(() => {
      ...
      //  第4引数に devicePixelRatio(= 真のディスプレイ倍率 webViewDpr) を添えて、native 側の applyDisplayScale が
      //  Linux 埋め込み時のウィンドウ物理サイズ補正（transform = webViewDpr / peerScale）に使えるようにする。
  ...
  //  バックプレッシャ方式: host への resize は往復処理（callNative の Promise は VST3 の onSize
  //  完了で解決）で、完了を待たず高頻度に送ると要求が積み上がってウィンドウがカーソルから
